...
class PairMLPEncoder(nn.Module):
    def __init__(self, x_dim, h_dim1=128, h_dim2=128, z_dim=10):
        super(PairMLPEncoder, self).__init__()

        self.backbone = nn.Sequential(nn.Linear(x_dim, h_dim1),
                                      nn.ReLU(),
    ...
    def forward(self, x):
        ...
        mu1= self.backbone(x1)
        mu2= self.backbone(x2)
        # mu = torch.cat((mu1, mu2), dim = -1)
        return mu1, mu2
...
class MLPClassifier(nn.Module):
    def __init__(self, x_dim, h_dim1=256, h_dim2=256, out_dim=20):
