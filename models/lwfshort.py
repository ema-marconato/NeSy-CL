...
class Lwfshort(ContinualModel):
    NAME = 'lwf'
    COMPATIBILITY = ['class-il', 'task-il']

    def __init__(self, backbone, loss, args, transform):
        super(Lwfshort, self).__init__(backbone, loss, args, transform)
        self.old_net = None
    ...
    def begin_task(self, dataset):
        ...
        if self.current_task > 0:
            # warm-up
            if not self.args.version == 'nesy': #doesn't work for nesy
                self.cbm=True
