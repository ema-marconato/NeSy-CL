...
class CLEVR(torch.utils.data.Dataset):
    def __init__(self, base_path):
        ...
        self.list_images= glob.glob(os.path.join(self.base_path,"image_TEST","*"))
        ...
        for item in range(len(self.list_images)):
            target_id=os.path.join(self.base_path,"scene_TEST",str(self.img_number[item])+".joblib")
    ...
    def images_folder(self):
        return os.path.join(self.base_path,"image_TEST")

    @property
    def scenes_path(self):
        return os.path.join(self.base_path,"scene_TEST")
    ...
    def __getitem__(self, item):
        ...
        image_id=os.path.join(self.base_path,"image_TEST",str(img_id)+".png")
...
for idx2,data in enumerate(loader):
    ...
    for idx,img in enumerate(predictions):
        ...
        if size>=2:
            ...
            new_image.save(os.path.join(dest,"image",str(number)+".jpg"))
            meta={"concepts":img_concept,"target":target}
            joblib.dump(meta,os.path.join(dest,os.path.join(dest,"meta",str(number)+".joblib")))
            number+=1
