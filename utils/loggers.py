...
def print_mean_accuracy(mean_acc: np.ndarray, mean_c_acc: np.ndarray, accs: list,
                        task_number: int, setting: str) -> None:
    """
    ...
    if setting == 'domain-il':
        mean_acc, _ = mean_acc
        print('\nLabels Accuracy for {} task(s): {} %'.format(
            task_number, round(mean_acc, 2)), file=sys.stderr)
        print('\nConcepts Accuracy for {} task(s): {} %'.format(
            task_number, round(mean_c_acc, 2)), file=sys.stderr)
    else:
        mean_acc_class_il, mean_acc_task_il = mean_acc
        c_mean_acc_class_il, c_mean_acc_task_il = mean_c_acc

        print('\nLabels Accuracy for {} task(s): \t [Class-IL]: {} %'
              ' \t [Task-IL]: {} % \t [Task]: {} %'.format(task_number, round(
            mean_acc_class_il, 2), round(mean_acc_task_il, 2), round(accs[0][task_number-1], 2)), file=sys.stderr)
        print('Concepts Accuracy for {} task(s): \t [Class-IL]: {} %'
              ' \t [Task-IL]: {} % \t [Task]: {} %\n'.format(task_number, round(
            c_mean_acc_class_il, 2), round(c_mean_acc_task_il, 2), round(accs[2][task_number-1], 2)), file=sys.stderr)

class CsvLogger:
    def __init__(self, setting_str: str, dataset_str: str,
                 model_str: str,
                 version: str, c_sup: float) -> None:
        self.accs = []
        self.c_accs = []
        if setting_str == 'class-il':
            self.accs_mask_classes = []
            self.c_accs_mask_classes = []
    ...
    def log(self, mean_acc: np.ndarray,
            c_mean_acc: np.ndarray) -> None:
        """
        ...
        if self.setting == 'general-continual':
            ...
            mean_acc_class_il, mean_acc_task_il = mean_acc
            c_mean_acc_class_il, c_mean_acc_task_il = c_mean_acc
            self.accs.append(mean_acc_class_il)
            self.accs_mask_classes.append(mean_acc_task_il)
            self.c_accs.append(c_mean_acc_class_il)
            self.c_accs_mask_classes.append(c_mean_acc_task_il)
    ...
    def write(self, args: Dict[str, Any]) -> None:
        ...
        # add acc on concepts
        for i, acc in enumerate(self.c_accs):
            args['task-c' + str(i + 1)] = acc
            new_cols.append('task-c' + str(i + 1))
        ...
        create_if_not_exists(base_path() + "results/" + self.setting +"/" + self.dataset)
        create_if_not_exists(base_path() + "results/" + self.setting +
                             "/" + self.dataset + "/" + self.model + f'-{self.version}-{self.c_sup}')

        write_headers = False
        path = base_path() + "results/" + self.setting + "/" + self.dataset\
               + "/" + self.model + f'-{self.version}-{self.c_sup}' + "/mean_accs.csv"
        ...
        if self.setting == 'class-il':
            ...
            # add acc on concepts
            for i, acc in enumerate(self.c_accs_mask_classes):
                args['task-c' + str(i + 1)] = acc
                new_cols.append('task-c' + str(i + 1))
